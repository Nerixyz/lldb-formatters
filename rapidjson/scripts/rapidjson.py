...
def GenericValueSummaryProvider(
    ...
    type_tag = flags & 0x7
    if type_tag == Type.kNullType:
        return "null"
    elif (
        type_tag == Type.kFalseType
        or type_tag == Type.kTrueType
        or type_tag == Type.kNumberType
    ):
        return ""
    elif type_tag == Type.kObjectType:
        return f"{{ size={valobj.GetNumChildren()} }}"
    elif type_tag == Type.kArrayType:
        return f"[ size={valobj.GetNumChildren()} ]"
    elif type_tag == Type.kStringType:
        return StringSummary(data, flags)
    else:
        return f"Unknown type {type_tag} (flags={flags:#x})"
...
class GenericValueSyntheticProvider:
    ...
    def update(self):
        ...
        if self._type == Type.kNullType:
            pass
        elif self._type == Type.kFalseType:
            self._val = self._valobj.CreateBoolValue("", False)
        elif self._type == Type.kTrueType:
            self._val = self._valobj.CreateBoolValue("", True)
        elif self._type == Type.kNumberType:
            n = data.GetChildMemberWithName("n")
            if (flags & Flags.kNumberIntFlag) == Flags.kNumberIntFlag:
                self._val = n.GetChildMemberWithName("i").GetChildMemberWithName("i")
            elif (flags & Flags.kNumberUintFlag) == Flags.kNumberUintFlag:
                self._val = n.GetChildMemberWithName("u").GetChildMemberWithName("u")
        ...
        elif self._type == Type.kArrayType:
        ...
        elif self._type == Type.kStringType:
            pass
